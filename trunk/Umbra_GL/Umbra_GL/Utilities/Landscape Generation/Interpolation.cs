...
namespace Umbra.Utilities
{
    static public class Interpolation
    {
        ...
        {
            float relativeX = (x - 0.5F) * 2.0F;
            float relativeY = (y - 0.5F) * 2.0F;

            int exponent = 4;

            float returnVal = (float)((-0.1F * Math.Pow(relativeX, exponent) + 0.1F) * (-0.1F * Math.Pow(relativeY, exponent) + 0.1F));

            return returnVal * 2000F;
        }
